...
@pytest.fixture
def seed_context(tmp_store, seed_doc):
...
class TestCreateDocument:
    ...
    def test_writes_context_file(self, tmp_store):
        bro_tools._do_create_document("s1", "comp-a", "v2")
        ctx = bro_tools._read_context("s1")
        assert ctx == {"component": "comp-a", "version": "v2"}

    def test_returns_success_string(self, tmp_store):
...
class TestSetDocumentContext:
    def test_switches_context_for_existing_doc(self, seed_doc, tmp_store):
    ...
    def test_errors_for_missing_doc(self, tmp_store):
...
class TestGetDocumentStatus:
    ...
    def test_errors_without_context(self, tmp_store):
...
class TestListDocuments:
    ...
    def test_empty_when_no_docs(self, tmp_store):
        result = bro_tools._do_list_documents()
...
class TestUpdateSection:
    ...
    def test_errors_on_bad_json(self, seed_context):
...
class TestSetSectionStatus:
    def test_updates_status(self, seed_context, tmp_store):
        result = bro_tools._do_set_section_status("test-session", "01-preface", "in_progress")
        assert "in_progress" in result
        # Verify on disk
        doc = tmp_store.get_document(*seed_context)
        assert doc.sections["01-preface"].status.value == "in_progress"

    def test_errors_on_invalid_status(self, seed_context):
        result = bro_tools._do_set_section_status("test-session", "01-preface", "bogus_status")
        assert "Error" in result
...
class TestExportMarkdown:
    def test_returns_non_empty_for_seeded_doc(self, seed_context):
...
class TestEntityLifecycle:
    ...
    def test_delete_entity_not_found(self, seed_context):
        result = bro_tools._do_delete_entity("test-session", "ghost")
        assert "Error" in result
        assert "not found" in result

    def test_list_entities_empty(self, seed_context):
        result = bro_tools._do_list_entities("test-session")
